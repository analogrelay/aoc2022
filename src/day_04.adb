...
procedure Day_04 is
   Dash_Set  : constant Character_Set := To_Set ('-');
   ...
   begin
      ...
      return
        (Min => Integer'Value (To_String (Head (Input, Dash_Index - 1))),
         Max =>
           Integer'Value
             (To_String (Tail (Input, Length (Input) - Dash_Index))));
   end Parse_Assignment;

   function Left_Contains_Right
     (Left : Assignment; Right : Assignment) return Boolean
   is
   begin
      return
        Left.Min >= Right.Min and then Left.Min <= Right.Max
        and then Left.Max >= Right.Min and then Left.Max <= Right.Max;
   end Left_Contains_Right;

   function One_Contains_Other
     (First_Assignment : Assignment; Second_Assignment : Assignment)
      return Boolean
   is
   begin
      return
        Left_Contains_Right (First_Assignment, Second_Assignment)
        or else Left_Contains_Right (Second_Assignment, First_Assignment);
   ...
   begin
      ...
      while not End_Of_File (F) loop
         ...
         begin
            ...
            First_Assignment  :=
              Parse_Assignment (Head (Line, Comma_Index - 1));
            Second_Assignment :=
